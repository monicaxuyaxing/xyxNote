...
@interface xyxGuideViewManager()<UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate>
...
@implementation xyxGuideViewManager

+ (instancetype)sharedInstance {
    static xyxGuideViewManager *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [xyxGuideViewManager new];
    });
    return instance;
}
...
- (UICollectionView *)view {
    if (!_view) {
        ...
        layout.itemSize = kxyxGuideViewBounds.size;
        layout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        
        _view = [[UICollectionView alloc] initWithFrame:kxyxGuideViewBounds collectionViewLayout:layout];
        _view.bounces = NO;
        ...
        [_view registerClass:[xyxCollectionViewCell class] forCellWithReuseIdentifier:cellID];
    }
    return _view;
}
...
- (UIPageControl *)pageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.frame = CGRectMake(0, 0, kxyxGuideViewBounds.size.width, 44.0f);
        _pageControl.center = CGPointMake(kxyxGuideViewBounds.size.width / 2, kxyxGuideViewBounds.size.height - 60);
    }
    return _pageControl;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    xyxCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellID forIndexPath:indexPath];
    
    UIImage *img = [self.images objectAtIndex:indexPath.row];
    CGSize size = [self adapterSizeImageSize:img.size compareSize:kxyxGuideViewBounds.size];
    
    //自适应图片位置,图片可以是任意尺寸,会自动缩放.
    cell.imageView.frame = CGRectMake(0, 0, size.width, size.height);
    cell.imageView.image = img;
    cell.imageView.center = CGPointMake(kxyxGuideViewBounds.size.width / 2, kxyxGuideViewBounds.size.height / 2);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    self.pageControl.currentPage = (scrollView.contentOffset.x / kxyxGuideViewBounds.size.width);
}
